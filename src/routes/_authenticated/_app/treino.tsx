...
type Exercise  = { id: string; name: string; sets: number | null; reps: number | null; position: number };
type Exercises = Partial<Record<keyof Schedule, Exercise[]>>;
...
function Treino() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      (supabase.from as any)("workout_exercises")
        .select("id,day_key,name,sets,reps,position")
        .eq("user_id", user.id)
        ...
        .then(({ data }: any) => {
          ...
          for (const ex of data as (Exercise & { day_key: keyof Schedule })[]) {
            if (!grouped[ex.day_key]) grouped[ex.day_key] = [];
            grouped[ex.day_key]!.push(ex);
          }
  ...
  async function deleteExercise(key: keyof Schedule, id: string) {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const { error } = await (supabase.from as any)("workout_exercises").delete().eq("id", id);
    if (error) { toast.error(error.message); return; }
              ...
              {pendingReceived.map((inv) => (
                <div key={inv.id} className="rounded-2xl border border-primary/20 bg-primary/5 p-4">
                  ...
                  <div className="flex gap-2">
                    <button onClick={() => respondInvite(inv.id, "accepted", inv)} className="flex-1 rounded-xl py-2 text-xs font-semibold text-white bg-gradient-primary shadow-glow active:scale-95 transition-all">Aceitar</button>
                    <button onClick={() => respondInvite(inv.id, "declined", inv)} className="flex-1 rounded-xl py-2 text-xs font-semibold text-muted-foreground border border-border active:scale-95 transition-all">Recusar</button>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="fixed inset-0 z-50 flex items-end bg-black/60 backdrop-blur-sm" onClick={() => setShowInviteModal(false)}>
          ...
          >
            ...
            {invStep === 2 && selMatch && (
              <>
                <div className="flex items-center gap-3 mb-5">
                  <button onClick={() => setInvStep(1)} className="grid h-9 w-9 place-items-center rounded-xl bg-white/8 text-muted-foreground active:scale-90 transition-all shrink-0">
                ...
                <div className="space-y-3">
                  ...
                  <div>
                    ...
                    {selMatch.gym && (
                      <button onClick={() => setInvAcademy(selMatch.gym!)} className="mt-1.5 flex items-center gap-1 text-xs text-primary active:opacity-70">
                        <MapPin className="h-3 w-3" /> Usar academia do match: {selMatch.gym}
                      </button>
                    )}
                  </div>
                  <button onClick={sendInvite} disabled={!invDate || !invTime || !invAcademy.trim() || sending}
